  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-orange-100 via-orange-50 to-white px-4 py-6 md:px-8 md:py-8 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800">
      <section className="mx-auto w-full max-w-7xl space-y-6">
        <header className="rounded-2xl border border-orange-200 bg-white p-5 shadow-lg shadow-orange-100/70 md:p-6 dark:border-slate-700 dark:bg-slate-800">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-2xl font-bold tracking-tight text-slate-900 md:text-3xl dark:text-white">
                Dashboard administrativo da aplicação
              </h1>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">Sessão ativa: {adminEmail || loginEmail}</p>
            </div>
            <button
              type="button"
              onClick={() => { if (window.confirm('Deseja realmente encerrar a sessão?')) handleLogout(); }}
              className="rounded-lg border border-slate-300 bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
            >
              Encerrar sessão
            </button>
          </div>
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          <article className="rounded-2xl border border-orange-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Usuários recentes</h2>
            <div className="mt-4 space-y-2">
              {recentUsers.length === 0 ? (
                <p className="text-sm text-slate-500 dark:text-slate-400">Sem registros recentes.</p>
              ) : (
                recentUsers.map((user) => (
                  <div key={user.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-600 dark:bg-slate-700/50">
                    <p className="text-sm font-medium text-slate-900 dark:text-white">{user.email}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{formatDate(user.createdAt)}</p>
                  </div>
                ))
              )}
            </div>
          ...
          <article className="rounded-2xl border border-orange-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Planos atualizados</h2>
            <div className="mt-4 space-y-2">
              {recentPlans.length === 0 ? (
                <p className="text-sm text-slate-500 dark:text-slate-400">Sem registros recentes.</p>
              ) : (
                recentPlans.map((plan) => (
                  <div key={plan.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-600 dark:bg-slate-700/50">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{plan.title}</p>
                      <span className="rounded-full bg-white px-2 py-0.5 text-xs text-slate-700 dark:bg-slate-600 dark:text-slate-200">
                        {plan.isActive ? 'Ativo' : 'Inativo'}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{plan.user?.email || '-'}</p>
                  </div>
                ))
              )}
            </div>
